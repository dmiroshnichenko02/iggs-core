...
?>
<section class="h-[1072px] py-[90px] overflow-hidden" style="background-image:url('<?= esc_url($background) ?>');background-size:cover;background-position:center;">
    <div class="container mx-auto">
        <div class="w-full flex justify-between">
           <div class="max-w-[577px]">
                <h2 class="text-h2 leading-h2 font-archivo font-light uppercase text-white mb-3"><?= esc_html($title) ?></h2>
                <div class="font-archivo text-medium leading-medium text-white font-light mb-[30px]"><?= wp_kses_post($subtitle) ?></div>
            ...
            <?php if ($logotype): ?>
                <img src="<?= esc_url($logotype) ?>" alt="Venue logo" class="h-[105px] w-[240px] object-cover" />
            <?php endif; ?>
        </div>
    </div>
    <?php if (is_array($images) && count($images) > 0): ?>
        <div class="pl-[calc((100%-1296px)/2)] relative mt-[45px]">
                    ...
                    <?php foreach ($images as $img):
                        ...
                        <?php if ($src): ?>
                            <div class="swiper-slide">
                                <img src="<?= esc_url($src) ?>" alt="Venue gallery image" class="object-cover h-[564px] w-full" style="max-width:unset;"/>
                            </div>
                        <?php endif; ?>
                    <?php endforeach; ?>
                </div>
            ...
            <div class="absolute bottom-[-104px] right-[calc((100%-1296px)/2)] flex gap-4 z-20">
                    <button class="venue-swiper-button-prev w-[63px] h-[63px] flex items-center justify-center rounded-full cursor-pointer bg-[#F5F5F5] hover:bg-[#BCBCBC]" type="button">
                    ...
                    <button class="venue-swiper-button-next w-[63px] h-[63px] flex items-center justify-center rounded-full cursor-pointer bg-[#F5F5F5] hover:bg-[#BCBCBC]" type="button">
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" xmlns="http://www.w3.org/2000/svg">
